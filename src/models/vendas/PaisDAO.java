...
public class PaisDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.vdPais = s.vdPais+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "vdPais") + 1;
			
			pais.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			pais.setCodigo(codigo);
			pais.setUtctag(utctag);
			pais.setCheckDelete(Util.checkDeleteCreate());
			pais.setFlagAtivo(1);
			pais.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        pais.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Pais> getList(List<Integer> flagAtivo) {
		List<Pais> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("PaisAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.PAIS))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Pais> list = em.createNamedQuery("PaisLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.PAIS))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Pais> list = em.createNamedQuery("PaisById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			paisPK.setCodigo(pais.getCodigo());
			paisPK.setCheckDelete(pais.getCheckDelete());

		    Pais newPais = em.find(Pais.class, paisPK);
			
			if(newPais != null && newPais.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Pais u "
					+ "SET u.flagAtivo = "+pais.getFlagAtivo()+","
			...
			.setParameter("recordNo", pais.getRecordNo())
			.setParameter("codemp", pais.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (pais.getFlagAtivo() == 1) {
			...
			auditoria.setHistorico(pais.getCodigo() + "-" + pais.getDescricao());
			auditoria.setDocCodigo(String.valueOf(pais.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Pais> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Pais> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT p FROM Pais p WHERE p." + column
