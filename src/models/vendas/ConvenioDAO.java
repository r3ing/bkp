...
public class ConvenioDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.vdConvenio = s.vdConvenio+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "vdConvenio") + 1;
			
			convenio.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			convenio.setCodigo(codigo);
			convenio.setUtctag(utctag);
			convenio.setCheckDelete(Util.checkDeleteCreate());
			convenio.setFlagAtivo(1);
			convenio.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        convenio.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Convenio> getList(List<Integer> flagAtivo) {
		List<Convenio> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("ConvenioAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.CONVENIO))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Convenio> list = em.createNamedQuery("ConvenioLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.CONVENIO))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Convenio> list = em.createNamedQuery("ConvenioById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			ConvenioPK convenioPK = new ConvenioPK();
			convenioPK.setCodigo(convenio.getCodigo());
//			convenioPK.setCodemp(convenio.getCodemp());
			convenioPK.setCheckDelete(convenio.getCheckDelete());

		    Convenio newConvenio = em.find(Convenio.class, convenioPK);
			
			if(newConvenio != null && newConvenio.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Convenio u "
					+ "SET u.flagAtivo = "+convenio.getFlagAtivo()+","
			...
			.setParameter("codigo", convenio.getCodigo())
			.setParameter("recordNo", convenio.getRecordNo())
			.setParameter("codemp", convenio.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (convenio.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadConvenio").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(convenio.getCodigo() + "-" + convenio.getDescricao());
			auditoria.setDocCodigo(String.valueOf(convenio.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Convenio> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Convenio> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT c FROM Convenio c WHERE c." + column
