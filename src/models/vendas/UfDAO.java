...
public class UfDAO {
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.vdUF = s.vdUF+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "vdUF") + 1;
			
			uf.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			uf.setCodigo(codigo);
			uf.setUtctag(utctag);
			uf.setCheckDelete(Util.checkDeleteCreate());
			uf.setFlagAtivo(1);
			uf.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
			uf.setLastMovto(ts_now);
	...
	public List<Uf> getList(List<Integer> flagAtivo) {
		List<Uf> list = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			//listUf = em.createNamedQuery("UfAll").getResultList();
			
			list = em.createNamedQuery("UfAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.UF))
					.getResultList();		
	...
	@SuppressWarnings("unchecked")
	public List<Uf> filterByColumn (String column, String filter, int action, List<Integer> ativo) throws Exception {
		 					
		List<Uf> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM Uf d WHERE d." + column
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Pais> list = em.createNamedQuery("UfLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.UF))
					.getResultList();
	...
	/**
	 * Metodo para buscar uf pelo CODIGO
	 * 
	 * @param codigo
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Uf> list = em.createNamedQuery("UfById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			em.getTransaction().begin();
			UfPK ufPK = new UfPK();
			ufPK.setCodigo(uf.getCodigo());
			ufPK.setCheckDelete(uf.getCheckDelete());

		    Uf newUf = em.find(Uf.class, ufPK);
			
			if(newUf != null && newUf.getFlagAtivo() == 1){
...
public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Uf u "
					+ "SET u.flagAtivo = "+uf.getFlagAtivo()+","
			...
			.setParameter("codigo", uf.getCodigo())
			.setParameter("recordNo", uf.getRecordNo())
			.setParameter("codemp", uf.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (uf.getFlagAtivo() == 1) {
			...
			auditoria.setHistorico(uf.getCodigo() + "-" + uf.getDescricao());
			auditoria.setDocCodigo(String.valueOf(uf.getCodigo()));
