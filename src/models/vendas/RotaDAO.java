...
public class RotaDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.vdRota = s.vdRota+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "vdRota") + 1;
			
			rotas.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			rotas.setCodigo(codigo);
			rotas.setUtctag(utctag);
			rotas.setCheckDelete(Util.checkDeleteCreate());
			rotas.setFlagAtivo(1);
			rotas.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        rotas.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Rota> getList(List<Integer> flagAtivo) {
		List<Rota> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("RotaAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.ROTAS))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Rota> list = em.createNamedQuery("RotaLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.ROTAS))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Rota> list = em.createNamedQuery("RotaById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			RotaPK rotasPK = new RotaPK();
			rotasPK.setCodigo(rotas.getCodigo());
//			rotasPK.setCodemp(rotas.getCodemp());
			rotasPK.setCheckDelete(rotas.getCheckDelete());

		    Rota newRota = em.find(Rota.class, rotasPK);
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Rota u "
					+ "SET u.flagAtivo = "+rotas.getFlagAtivo()+","
			...
			.setParameter("codigo", rotas.getCodigo())
			.setParameter("recordNo", rotas.getRecordNo())
			.setParameter("codemp", rotas.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (rotas.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadRotas").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(rotas.getCodigo() + "-" + rotas.getDescricao());
			auditoria.setDocCodigo(String.valueOf(rotas.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Rota> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Rota> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT r FROM Rota r WHERE r." + column
