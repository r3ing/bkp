...
public class FuncionarioDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.rhFuncionario = s.rhFuncionario+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "rhFuncionario") + 1;
			
			funcionario.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			funcionario.setCodigo(codigo);
			funcionario.setUtctag(utctag);
			funcionario.setCheckDelete(Util.checkDeleteCreate());
			funcionario.setFlagAtivo(1);
			funcionario.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        funcionario.setLastMovto(ts_now);
	...
	/**
	 * Metodo para listar tudos os FUCIONARIOS, dependendo do valor do
	 * flag_Ativo e compartilhamento
	 * 
	 * @param flagAtivo
	 * @return
	 */
	public List<Funcionario> getList(List<Integer> flagAtivo) {
		List<Funcionario> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("FuncionarioAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FUNCIONARIO))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Funcionario> list = em.createNamedQuery("FuncionarioLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FUNCIONARIO))
					.getResultList();
	...
	/**
	 * Metodo para buscar Funcionario pelo CODIGO
	 * 
	 * @param codigo
	 * @return funcionario buscado ou null
	 */
	@SuppressWarnings("unchecked")
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Funcionario> list = em.createNamedQuery("FuncionarioById")
	...
	/**
	 * Metodo para buscar Vendedor pelo CODIGO
	 * 
	 * @param codigo
	 * @return Funcionario buscado ou null
	 */
	@SuppressWarnings("unchecked")
	public LogRetorno getVencedorById(Integer codigo) {
		...
		try{
			...
			List<Funcionario> list = em.createNamedQuery("VendedorById")
					.setParameter("codigo", codigo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FUNCIONARIO))
					.getResultList();
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			FuncionarioPK funcionarioPK = new FuncionarioPK();
			funcionarioPK.setCodigo(funcionario.getCodigo());
			funcionarioPK.setCheckDelete(funcionario.getCheckDelete());

		    Funcionario newFuncionario = em.find(Funcionario.class, funcionarioPK);
			
			if(newFuncionario != null && newFuncionario.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Funcionario u "
					+ "SET u.flagAtivo = "+funcionario.getFlagAtivo()+","
			...
			.setParameter("codigo", funcionario.getCodigo())
			.setParameter("recordNo", funcionario.getRecordNo())
			.setParameter("codemp", funcionario.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (funcionario.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadFuncionario").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(funcionario.getCodigo() + "-" + funcionario.getDescricao());
			auditoria.setDocCodigo(String.valueOf(funcionario.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Funcionario> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Funcionario> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT f FROM Funcionario f WHERE f." + column
	...
	@SuppressWarnings("unchecked")
	public List<Funcionario> filterVendedorByColumn (String column, String filter, int action, List<Integer> ativo) {
		...
		try {
			...
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT f FROM Funcionario f WHERE f." + column
									+ " LIKE CONCAT('%', :filter, '%') AND f.codemp IN (:codemp) AND f.flagTipoFuncionario = 0 AND f.flagAtivo IN (:flag)")
							.setParameter("filter", (filter.equals("")) ? "%" : filter)
							.setParameter("codemp", Util.getCompartilhamentoEntidade(EnumCompartilhamento.FUNCIONARIO))
							.setParameter("flag", ativo)
							.getResultList();
					...
					listFilter = em
							.createQuery("SELECT f FROM Funcionario f WHERE f." + column
									+ "= :filter AND f.codemp IN (:codemp) AND f.flagTipoFuncionario = 0 AND f.flagAtivo IN (:flag)")
							.setParameter("filter", Integer.parseInt(filter))
							.setParameter("codemp", Util.getCompartilhamentoEntidade(EnumCompartilhamento.FUNCIONARIO))
							.setParameter("flag", ativo)
							.getResultList();
