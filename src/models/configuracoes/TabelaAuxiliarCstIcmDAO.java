...
public class TabelaAuxiliarCstIcmDAO
{
	...
	public LogRetorno insert(Object objeto, String cst)
	{
		...
		TabelaAuxiliarCstIcm entity = (TabelaAuxiliarCstIcm) objeto;
	 ...
	 * @return
	 */
	public List<TabelaAuxiliarCstIcm> getList(List<Integer> flagAtivo) 
	{
		List<TabelaAuxiliarCstIcm> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("TabelaAuxiliarCstIcmAll")
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<TabelaAuxiliarCstIcm> list = em.createNamedQuery("TabelaAuxiliarCstIcmLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.LAYOUT_ARQUIVO_CAB))
					.getResultList();
	...
	public LogRetorno getById(String codigo) 
	{
		...
		try
		{
			...
			List<TabelaAuxiliarCstIcm> list = em.createNamedQuery("TabelaAuxiliarCstIcmById")
	...
	{		
		   ...
		   try 
		   {
				...
				/*em.createQuery("UPDATE TabelaAuxiliarCstIcm t SET t.cst = :cst, t.descricao = :descricao WHERE t.codemp = :codemp AND t.checkDelete = :checkDelete")
	...
	{		
		...
		try 
		{
			...
			em.createQuery("UPDATE TabelaAuxiliarCstIcm u "
	...
	@SuppressWarnings("unchecked")
	public List<TabelaAuxiliarCstIcm> filterByColumn (String column, String filter, int action, List<Integer> ativo) 
	{		 					
		List<TabelaAuxiliarCstIcm> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM TabelaAuxiliarCstIcm d WHERE d." + column
