...
public class TabelaAuxiliarCstPiDAO
{
	...
	public LogRetorno insert(Object objeto, String cst)
	{
		...
		TabelaAuxiliarCstPi entity = (TabelaAuxiliarCstPi) objeto;
	 ...
	 * @return
	 */
	public List<TabelaAuxiliarCstPi> getList(List<Integer> flagAtivo) 
	{
		List<TabelaAuxiliarCstPi> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("TabelaAuxiliarCstPiAll")
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<TabelaAuxiliarCstPi> list = em.createNamedQuery("TabelaAuxiliarCstPiLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.LAYOUT_ARQUIVO_CAB))
					.getResultList();
	...
	public LogRetorno getById(String codigo) 
	{
		...
		try
		{
			...
			List<TabelaAuxiliarCstPi> list = em.createNamedQuery("TabelaAuxiliarCstPiById")
	...
	{		
		   ...
		   try 
		   {
				...
				/*em.createQuery("UPDATE TabelaAuxiliarCstPi t SET t.cst = :cst, t.descricao = :descricao WHERE t.codemp = :codemp AND t.checkDelete = :checkDelete")
	...
	{		
		...
		try 
		{
			...
			em.createQuery("UPDATE TabelaAuxiliarCstPi u "
	...
	@SuppressWarnings("unchecked")
	public List<TabelaAuxiliarCstPi> filterByColumn (String column, String filter, int action, List<Integer> ativo) 
	{		 					
		List<TabelaAuxiliarCstPi> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM TabelaAuxiliarCstPi d WHERE d." + column
