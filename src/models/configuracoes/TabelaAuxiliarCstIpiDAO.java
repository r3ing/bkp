...
public class TabelaAuxiliarCstIpiDAO
{
	...
	public LogRetorno insert(Object objeto, String cst)
	{
		...
		TabelaAuxiliarCstIpi entity = (TabelaAuxiliarCstIpi) objeto;
	 ...
	 * @return
	 */
	public List<TabelaAuxiliarCstIpi> getList(List<Integer> flagAtivo) 
	{
		List<TabelaAuxiliarCstIpi> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("TabelaAuxiliarCstIpiAll")
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<TabelaAuxiliarCstIpi> list = em.createNamedQuery("TabelaAuxiliarCstIpiLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.LAYOUT_ARQUIVO_CAB))
					.getResultList();
	...
	public LogRetorno getById(String codigo) 
	{
		...
		try
		{
			...
			List<TabelaAuxiliarCstIpi> list = em.createNamedQuery("TabelaAuxiliarCstIpiById")
	...
	{		
		   ...
		   try 
		   {
				...
				/*em.createQuery("UPDATE TabelaAuxiliarCstIpi t SET t.cst = :cst, t.descricao = :descricao WHERE t.codemp = :codemp AND t.checkDelete = :checkDelete")
	...
	{		
		...
		try 
		{
			...
			em.createQuery("UPDATE TabelaAuxiliarCstIpi u "
	...
	@SuppressWarnings("unchecked")
	public List<TabelaAuxiliarCstIpi> filterByColumn (String column, String filter, int action, List<Integer> ativo) 
	{		 					
		List<TabelaAuxiliarCstIpi> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM TabelaAuxiliarCstIpi d WHERE d." + column
