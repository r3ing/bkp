...
public class TributacaoDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		Tributacao tributacao = (Tributacao) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpTributacao = s.cpTributacao+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpTributacao") + 1;
			
			tributacao.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			tributacao.setCodigo(codigo);
			tributacao.setUtctag(utctag);
			tributacao.setCheckDelete(Util.checkDeleteCreate());
			tributacao.setFlagAtivo(1);
			tributacao.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        tributacao.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Tributacao> getList(List<Integer> flagAtivo) {
		List<Tributacao> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("TributacaoAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.TRIBUTACAO))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Tributacao> list = em.createNamedQuery("TributacaoLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.TRIBUTACAO))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Tributacao> list = em.createNamedQuery("TributacaoById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			TributacaoPK tributacaoPK = new TributacaoPK();
			tributacaoPK.setCodigo(tributacao.getCodigo());
			tributacaoPK.setCheckDelete(tributacao.getCheckDelete());

		    Tributacao newTributacao = em.find(Tributacao.class, tributacaoPK);
			
			if(newTributacao != null && newTributacao.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Tributacao u "
					+ "SET u.flagAtivo = "+tributacao.getFlagAtivo()+","
			...
			.setParameter("codigo", tributacao.getCodigo())
			.setParameter("checkDelete", tributacao.getCheckDelete())
			.setParameter("codemp", tributacao.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (tributacao.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadTributacao").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(tributacao.getCodigo() + "-" + tributacao.getDescricao());
			auditoria.setDocCodigo(String.valueOf(tributacao.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Tributacao> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Tributacao> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM Tributacao d WHERE d." + column
