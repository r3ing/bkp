...
public class FamiliaPrecoDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		FamiliaPreco familiaPreco = (FamiliaPreco) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpFamiliaPreco = s.cpFamiliaPreco+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpFamiliaPreco") + 1;
			
			familiaPreco.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			familiaPreco.setCodigo(codigo);
			familiaPreco.setUtctag(utctag);
			familiaPreco.setCheckDelete(Util.checkDeleteCreate());
			familiaPreco.setFlagAtivo(1);
			familiaPreco.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        familiaPreco.setLastMovto(ts_now);
			
			em.persist(familiaPreco);
	 ...
	 * @return
	 */
	public List<FamiliaPreco> getList(List<Integer> flagAtivo) {
		List<FamiliaPreco> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("FamiliaPrecoAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FAMILIAS_DE_PRECOS))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<FamiliaPreco> list = em.createNamedQuery("FamiliaPrecoLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FAMILIAS_DE_PRECOS))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<FamiliaPreco> list = em.createNamedQuery("FamiliaPrecoById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			FamiliaPrecoPK familiaPrecoPK = new FamiliaPrecoPK();
			familiaPrecoPK.setCodigo(familiaPreco.getCodigo());
			familiaPrecoPK.setCheckDelete(familiaPreco.getCheckDelete());

		    FamiliaPreco newFamiliaPreco = em.find(FamiliaPreco.class, familiaPrecoPK);
			
			if(newFamiliaPreco != null && newFamiliaPreco.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE FamiliaPreco u "
					+ "SET u.flagAtivo = "+familiaPreco.getFlagAtivo()+","
			...
			.setParameter("codigo", familiaPreco.getCodigo())
			.setParameter("checkDelete", familiaPreco.getCheckDelete())
			.setParameter("codemp", familiaPreco.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (familiaPreco.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadFamiliaPreco").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(familiaPreco.getCodigo() + "-" + familiaPreco.getDescricao());
			auditoria.setDocCodigo(String.valueOf(familiaPreco.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<FamiliaPreco> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<FamiliaPreco> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM FamiliaPreco d WHERE d." + column
