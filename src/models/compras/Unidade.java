...
@Table(name = "UNIDADE")
@NamedQueries({
		@NamedQuery(name = "UnidadeById", query = "SELECT d FROM Unidade d WHERE d.codigo =:codigo AND d.flagAtivo = 1 AND d.codemp IN (:codemp)"),
		@NamedQuery(name = "UnidadeAll", query = "SELECT d FROM Unidade d WHERE d.flagAtivo IN (:flag) AND d.codemp IN (:codemp)"),
		@NamedQuery(name = "UnidadeLast", query = "SELECT d FROM Unidade d WHERE d.codemp IN (:codemp) AND d.flagAtivo = 1 ORDER BY d.codigo DESC")
		})
@IdClass(value = UnidadePK.class)
