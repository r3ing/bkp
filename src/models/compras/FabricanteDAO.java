...
public class FabricanteDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpFabricante = s.cpFabricante+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpFabricante") + 1;
			
			fabricante.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			fabricante.setCodigo(codigo);
			fabricante.setUtctag(utctag);
			fabricante.setCheckDelete(Util.checkDeleteCreate());
			fabricante.setFlagAtivo(1);
			fabricante.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        fabricante.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Fabricante> getList(List<Integer> flagAtivo) {
		List<Fabricante> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("FabricanteAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FABRICANTES))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Fabricante> list = em.createNamedQuery("FabricanteLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.FABRICANTES))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Fabricante> list = em.createNamedQuery("FabricanteById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			FabricantePK fabricantePK = new FabricantePK();
			fabricantePK.setCodigo(fabricante.getCodigo());
			fabricantePK.setCheckDelete(fabricante.getCheckDelete());

		    Fabricante newFabricante = em.find(Fabricante.class, fabricantePK);
			
			if(newFabricante != null && newFabricante.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Fabricante u "
					+ "SET u.flagAtivo = "+fabricante.getFlagAtivo()+","
			...
			.setParameter("codigo", fabricante.getCodigo())
			.setParameter("checkDelete", fabricante.getCheckDelete())
			.setParameter("codemp", fabricante.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (fabricante.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadFabricante").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(fabricante.getCodigo() + "-" + fabricante.getDescricao());
			auditoria.setDocCodigo(String.valueOf(fabricante.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Fabricante> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Fabricante> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM Fabricante d WHERE d." + column
