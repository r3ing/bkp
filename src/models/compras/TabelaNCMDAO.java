...
public class TabelaNCMDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		TabelaNCM tabelaNCM = (TabelaNCM) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpTabelaNCM = s.cpTabelaNCM+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpTabelaNCM") + 1;
			
			tabelaNCM.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			tabelaNCM.setCodigo(codigo);
			tabelaNCM.setUtctag(utctag);
			tabelaNCM.setCheckDelete(Util.checkDeleteCreate());
			tabelaNCM.setFlagAtivo(1);
			tabelaNCM.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        tabelaNCM.setLastMovto(ts_now);
			
			em.persist(tabelaNCM);
	 ...
	 * @return
	 */
	public List<TabelaNCM> getList(List<Integer> flagAtivo) {
		List<TabelaNCM> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("TabelaNCMAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.TABELA_NCM))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<TabelaNCM> list = em.createNamedQuery("TabelaNCMLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.TABELA_NCM))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<TabelaNCM> list = em.createNamedQuery("TabelaNCMById")
	 ...
	 */
	@SuppressWarnings("unchecked")
	public LogRetorno getById(String codNCM) {
		...
		try{
			...
			List<TabelaNCM> list = em.createNamedQuery("TabelaNCMByNCM")
					.setParameter("codNCM", codNCM)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.TABELA_NCM))
					.getResultList();
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			TabelaNCMPK tabelaNCMPK = new TabelaNCMPK();
			tabelaNCMPK.setCodigo(tabelaNCM.getCodigo());
			tabelaNCMPK.setCheckDelete(tabelaNCM.getCheckDelete());

		    TabelaNCM newTabelaNCM = em.find(TabelaNCM.class, tabelaNCMPK);
			
			if(newTabelaNCM != null && newTabelaNCM.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE TabelaNCM u "
					+ "SET u.flagAtivo = "+tabelaNCM.getFlagAtivo()+","
			...
			.setParameter("codigo", tabelaNCM.getCodigo())
			.setParameter("checkDelete", tabelaNCM.getCheckDelete())
			.setParameter("codemp", tabelaNCM.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (tabelaNCM.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadTabelaNCM").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(tabelaNCM.getCodigo() + "-" + tabelaNCM.getDescricao());
			auditoria.setDocCodigo(String.valueOf(tabelaNCM.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<TabelaNCM> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<TabelaNCM> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM TabelaNCM d WHERE d." + column
