...
public class ItemValorDAO {
	...
	public void ins(Item i){
		em = ConnectionHib.emf.createEntityManager();
		ItemValor iv = new ItemValor();
		
		
	
		
		ItemPK pk = new ItemPK();
		pk.setCheckDelete(i.getCheckDelete());
		pk.setCodigo(i.getCodigo());
		...
		Item item = em.getReference(Item.class, pk);
		...
		iv.setCodemp(56);
		iv.setFlagAtivo(1);
		iv.setLastMovto(ts_now);
		iv.setUtctag(utctag);
		iv.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
		iv.setCodTributacao(69);
		em.persist(iv);
