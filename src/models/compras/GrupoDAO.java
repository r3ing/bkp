...
public class GrupoDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpGrupo = s.cpGrupo+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpGrupo") + 1;
			
			grupo.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			grupo.setCodigo(codigo);
			grupo.setUtctag(utctag);
			grupo.setCheckDelete(Util.checkDeleteCreate());
			grupo.setFlagAtivo(1);
			grupo.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        grupo.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Grupo> getList(List<Integer> flagAtivo) {
		List<Grupo> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("GrupoAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.GRUPOS))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Grupo> list = em.createNamedQuery("GrupoLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.GRUPOS))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Grupo> list = em.createNamedQuery("GrupoById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			grupoPK.setCodigo(grupo.getCodigo());
			grupoPK.setCheckDelete(grupo.getCheckDelete());

			Grupo newGrupo = em.find(Grupo.class, grupoPK);
			
			if(newGrupo != null && newGrupo.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Grupo u "
					+ "SET u.flagAtivo = "+grupo.getFlagAtivo()+","
			...
			.setParameter("codigo", grupo.getCodigo())
			.setParameter("checkDelete", grupo.getCheckDelete())
			.setParameter("codemp", grupo.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (grupo.getFlagAtivo() == 1) {
			...
			auditoria.setHistorico(grupo.getCodigo() + "-" + grupo.getDescricao());
			auditoria.setDocCodigo(String.valueOf(grupo.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Grupo> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Grupo> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM Grupo d WHERE d." + column
