...
public class BoxLocalEstoqueDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		BoxLocalEstoque boxLocalEstoque = (BoxLocalEstoque) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpBoxLocalEstoque = s.cpBoxLocalEstoque+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpBoxLocalEstoque") + 1;
			
			boxLocalEstoque.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			boxLocalEstoque.setCodigo(codigo);
			boxLocalEstoque.setUtctag(utctag);
			boxLocalEstoque.setCheckDelete(Util.checkDeleteCreate());
			boxLocalEstoque.setFlagAtivo(1);
			boxLocalEstoque.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        boxLocalEstoque.setLastMovto(ts_now);
			
			em.persist(boxLocalEstoque);
			...
			logRet.setObjeto(boxLocalEstoque);
	 ...
	 * @return
	 */
	public List<BoxLocalEstoque> getList(List<Integer> flagAtivo) {
		List<BoxLocalEstoque> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("BoxLocalEstoqueAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.BOX_LOCAL_ESTOQUE))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<BoxLocalEstoque> list = em.createNamedQuery("BoxLocalEstoqueLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.BOX_LOCAL_ESTOQUE))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<BoxLocalEstoque> list = em.createNamedQuery("BoxLocalEstoqueById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			BoxLocalEstoquePK boxLocalEstoquePK = new BoxLocalEstoquePK();
			boxLocalEstoquePK.setCodigo(boxLocalEstoque.getCodigo());
			boxLocalEstoquePK.setCheckDelete(boxLocalEstoque.getCheckDelete());

		    BoxLocalEstoque newBoxLocalEstoque = em.find(BoxLocalEstoque.class, boxLocalEstoquePK);
			
			if(newBoxLocalEstoque != null && newBoxLocalEstoque.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE BoxLocalEstoque u "
					+ "SET u.flagAtivo = "+boxLocalEstoque.getFlagAtivo()+","
			...
			.setParameter("codigo", boxLocalEstoque.getCodigo())
			.setParameter("checkDelete", boxLocalEstoque.getCheckDelete())
			.setParameter("codemp", boxLocalEstoque.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (boxLocalEstoque.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadBoxLocalEstoque").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(boxLocalEstoque.getCodigo() + "-" + boxLocalEstoque.getDescricao());
			auditoria.setDocCodigo(String.valueOf(boxLocalEstoque.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<BoxLocalEstoque> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<BoxLocalEstoque> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM BoxLocalEstoque d WHERE d." + column
