...
public class OperacaoEntradaDAO1 implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		OperacaoEntrada1 operacaoEntrada = (OperacaoEntrada1) objeto;
	 ...
	 * @return
	 */
	public List<OperacaoEntrada1> getList(List<Integer> flagAtivo) {
		List<OperacaoEntrada1> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<OperacaoEntrada1> list = em.createNamedQuery("OperacaoEntradaLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.OPERACOES_DE_ENTRADA))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<OperacaoEntrada1> list = em.createNamedQuery("OperacaoEntradaById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			OperacaoEntradaPK1 operacaoEntradaPK = new OperacaoEntradaPK1();
			operacaoEntradaPK.setCodigo(operacaoEntrada.getCodigo());
			operacaoEntradaPK.setCodemp(operacaoEntrada.getCodemp());
			operacaoEntradaPK.setCheckDelete(operacaoEntrada.getCheckDelete());

		    OperacaoEntrada1 newOperacaoEntrada = em.find(OperacaoEntrada1.class, operacaoEntradaPK);
	...
	@SuppressWarnings("unchecked")
	public List<OperacaoEntrada1> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<OperacaoEntrada1> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM OperacaoEntrada d WHERE d." + column
