...
public class CorEstiloDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		CorEstilo corEstilo = (CorEstilo) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.cpCorEstilo = s.cpCorEstilo+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "cpCorEstilo") + 1;
			
			corEstilo.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			corEstilo.setCodigo(codigo);
			corEstilo.setUtctag(utctag);
			corEstilo.setCheckDelete(Util.checkDeleteCreate());
			corEstilo.setFlagAtivo(1);
			corEstilo.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        corEstilo.setLastMovto(ts_now);
			
			em.persist(corEstilo);
	 ...
	 * @return
	 */
	public List<CorEstilo> getList(List<Integer> flagAtivo) {
		List<CorEstilo> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("CorEstiloAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.COR_ESTILOS))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<CorEstilo> list = em.createNamedQuery("CorEstiloLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.COR_ESTILOS))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<CorEstilo> list = em.createNamedQuery("CorEstiloById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			CorEstiloPK corEstiloPK = new CorEstiloPK();
			corEstiloPK.setCodigo(corEstilo.getCodigo());
			corEstiloPK.setCheckDelete(corEstilo.getCheckDelete());

		    CorEstilo newCorEstilo = em.find(CorEstilo.class, corEstiloPK);
			
			if(newCorEstilo != null && newCorEstilo.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE CorEstilo u "
					+ "SET u.flagAtivo = "+corEstilo.getFlagAtivo()+","
			...
			.setParameter("codigo", corEstilo.getCodigo())
			.setParameter("checkDelete", corEstilo.getCheckDelete())
			.setParameter("codemp", corEstilo.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (corEstilo.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadCorEstilo").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(corEstilo.getCodigo() + "-" + corEstilo.getDescricao());
			auditoria.setDocCodigo(String.valueOf(corEstilo.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<CorEstilo> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<CorEstilo> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT d FROM CorEstilo d WHERE d." + column
