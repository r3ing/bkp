...
public class PortadorDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		Portador portador = (Portador) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.fcPortador = s.fcPortador+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "fcPortador") + 1;
			
			portador.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			portador.setCodigo(codigo);
			portador.setUtctag(utctag);
			portador.setCheckDelete(Util.checkDeleteCreate());
			portador.setFlagAtivo(1);
			portador.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        portador.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Portador> getList(List<Integer> flagAtivo) {
		List<Portador> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("PortadorAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.PORTADOR))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Portador> list = em.createNamedQuery("PortadorLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.PORTADOR))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Portador> list = em.createNamedQuery("PortadorById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			PortadorPK portadorPK = new PortadorPK();
			portadorPK.setCodigo(portador.getCodigo());
//			portadorPK.setCodemp(portador.getCodemp());
			portadorPK.setCheckDelete(portador.getCheckDelete());

		    Portador newRota = em.find(Portador.class, portadorPK);
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Portador u "
					+ "SET u.flagAtivo = "+portador.getFlagAtivo()+","
			...
			.setParameter("codigo", portador.getCodigo())
			.setParameter("recordNo", portador.getRecordNo())
			.setParameter("codemp", portador.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (portador.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadPortador").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(portador.getCodigo() + "-" + portador.getDescricao());
			auditoria.setDocCodigo(String.valueOf(portador.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Portador> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Portador> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT r FROM Portador r WHERE r." + column
