...
public class SecaoDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.fcSecao = s.fcSecao+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "fcSecao") + 1;
			
			secao.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			secao.setCodigo(codigo);
			secao.setUtctag(utctag);
			secao.setCheckDelete(Util.checkDeleteCreate());
			secao.setFlagAtivo(1);
			secao.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        secao.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Secao> getList(List<Integer> flagAtivo) {
		List<Secao> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("SecaoAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.SECAO))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Secao> list = em.createNamedQuery("SecaoLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.SECAO))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Secao> list = em.createNamedQuery("SecaoById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			SecaoPK secaoPK = new SecaoPK();
			secaoPK.setCodigo(secao.getCodigo());
//			secaoPK.setCodemp(secao.getCodemp());
			secaoPK.setCheckDelete(secao.getCheckDelete());

		    Secao newSecao = em.find(Secao.class, secaoPK);
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Secao u "
					+ "SET u.flagAtivo = "+secao.getFlagAtivo()+","
			...
			.setParameter("codigo", secao.getCodigo())
			.setParameter("recordNo", secao.getRecordNo())
			.setParameter("codemp", secao.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (secao.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadSecao").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(secao.getCodigo() + "-" + secao.getDescricao());
			auditoria.setDocCodigo(String.valueOf(secao.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Secao> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Secao> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT r FROM Secao r WHERE r." + column
