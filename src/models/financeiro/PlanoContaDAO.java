...
public class PlanoContaDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		PlanoConta planoConta = (PlanoConta) objeto;
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.fcPlanoConta = s.fcPlanoConta+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "fcPlanoConta") + 1;
			
			planoConta.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			planoConta.setCodigo(codigo);
			planoConta.setUtctag(utctag);
			planoConta.setCheckDelete(Util.checkDeleteCreate());
			planoConta.setFlagAtivo(1);
			planoConta.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        planoConta.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<PlanoConta> getList(List<Integer> flagAtivo) {
		List<PlanoConta> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("PlanoContaAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.PLANO_DE_CONTA))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<PlanoConta> list = em.createNamedQuery("PlanoContaLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.PLANO_DE_CONTA))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<PlanoConta> list = em.createNamedQuery("PlanoContaById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			PlanoContaPK planoContaPK = new PlanoContaPK();
			planoContaPK.setCodigo(planoConta.getCodigo());
//			planoContaPK.setCodemp(planoConta.getCodemp());
			planoContaPK.setCheckDelete(planoConta.getCheckDelete());

		    PlanoConta newPlanoConta = em.find(PlanoConta.class, planoContaPK);
			
			if(newPlanoConta != null && newPlanoConta.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE PlanoConta u "
					+ "SET u.flagAtivo = "+planoConta.getFlagAtivo()+","
			...
			.setParameter("codigo", planoConta.getCodigo())
			.setParameter("recordNo", planoConta.getRecordNo())
			.setParameter("codemp", planoConta.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (planoConta.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadPlanoContas").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(planoConta.getCodigo() + "-" + planoConta.getDescricao());
			auditoria.setDocCodigo(String.valueOf(planoConta.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<PlanoConta> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<PlanoConta> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT r FROM PlanoConta r WHERE r." + column
