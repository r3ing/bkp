...
public class MoedaDAO implements InterfaceDAO{
	...
	public LogRetorno insert(Object objeto){
		...
		try {
			...
			em.createQuery("UPDATE Sequencial s SET s.fcMoeda = s.fcMoeda+1 WHERE s.codemp= :codemp AND s.checkDelete = :checkDelete")
			...
			Integer codigo = seqDAO.getLastInsertCodigo(DadosGlobais.empresaLogada.getCodigo(), "fcMoeda") + 1;
			
			moeda.setCodemp(DadosGlobais.empresaLogada.getCodigo());
			moeda.setCodigo(codigo);
			moeda.setUtctag(utctag);
			moeda.setCheckDelete(Util.checkDeleteCreate());
			moeda.setFlagAtivo(1);
			moeda.setLastCoduser(DadosGlobais.usuarioLogado.getCodigo());
	        moeda.setLastMovto(ts_now);
	 ...
	 * @return
	 */
	public List<Moeda> getList(List<Integer> flagAtivo) {
		List<Moeda> list = null;
		try {	
		em = ConnectionHib.emf.createEntityManager();		
		
		list = em.createNamedQuery("MoedaAll")
					.setParameter("flag", flagAtivo)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.MOEDA))
					.getResultList();
	...
	public LogRetorno getLast(){
		...
		try{
			...
			List<Moeda> list = em.createNamedQuery("MoedaLast")
					.setMaxResults(1)
					.setParameter("codemp",Util.getCompartilhamentoEntidade(EnumCompartilhamento.MOEDA))
					.getResultList();
	...
	public LogRetorno getById(Integer codigo) {
		...
		try{
			...
			List<Moeda> list = em.createNamedQuery("MoedaById")
	...
	public LogRetorno update(Object objeto){
		...
		try {
			...
			logRet.setStatus(EnumLogRetornoStatus.INICIOTRANSACAO);
			em.getTransaction().begin();
			MoedaPK moedaPK = new MoedaPK();
			moedaPK.setCodigo(moeda.getCodigo());
			moedaPK.setCheckDelete(moeda.getCheckDelete());

		    Moeda newMoeda = em.find(Moeda.class, moedaPK);
			
			if(newMoeda != null && newMoeda.getFlagAtivo() == 1){
	...
	public void delete(Object objeto) {
		...
		try {
			...
			em.createQuery("UPDATE Moeda u "
					+ "SET u.flagAtivo = "+moeda.getFlagAtivo()+","
			...
			.setParameter("codigo", moeda.getCodigo())
			.setParameter("checkDelete", moeda.getCheckDelete())
			.setParameter("codemp", moeda.getCodemp()).executeUpdate();
			
			Integer tipoOperacao = null;
			String valorAnterior = null;
			String valorNovo = null;
			if (moeda.getFlagAtivo() == 1) {
			...
			auditoria.setTipoRegistro(DadosGlobais.resourceBundle.getString("miCadMoeda").toUpperCase());
			auditoria.setDtMovto(Util.geraDataMovto(ts_now));
			auditoria.setValorAnterior(valorAnterior);
			auditoria.setValorNovo(valorNovo);
			auditoria.setHistorico(moeda.getCodigo() + "-" + moeda.getSimbolo());
			auditoria.setDocCodigo(String.valueOf(moeda.getCodigo()));
	...
	@SuppressWarnings("unchecked")
	public List<Moeda> filterByColumn (String column, String filter, int action, List<Integer> ativo) {
		 					
		List<Moeda> listFilter = null;
		try {
			em = ConnectionHib.emf.createEntityManager();
			if (action == 1) {
				if (filter.equals("")) {
					listFilter = em
							.createQuery("SELECT r FROM Moeda r WHERE r." + column
