...
import tools.enums.EnumTipoOperacaoFinaceiro;
...
public class OperacaoFinanceiroController implements Initializable {

	@FXML
	private AnchorPane anchorPanePrincipal;

	@FXML
	private AnchorPane anchorPaneListagem, anchorPaneContent;
	...
	@FXML
	private Label lblCboxFlagAtivo, lblErrorMessage;

	@FXML
	private CustomTextField txtFilterColumn;

	@FXML
	private TableView<OperacaoFinanceiro> tbView;

	@FXML
	private TableColumn<OperacaoFinanceiro, Integer> tbColCodigo;

	@FXML
	private TableColumn<OperacaoFinanceiro, String> tbColDescricao;

	@FXML
	private TableColumn<OperacaoFinanceiro, Integer> tbColAtivoInat;
	
	@FXML
	private TableColumn<OperacaoFinanceiro, Integer> tbColCodeEmp;
	
	@FXML
	private TableColumn<OperacaoFinanceiro, String> tbColObservacao;
    ...
    @FXML
    private ComboBox<ComboBoxFilter> cboxTipoOperacao;

    @FXML
    private Label lblFlagAutorizacao;

    @FXML
    private JFXToggleButton tgbFlagAutorizacao;
	...
	// ATRIBUTOS DA CLASSE
	private ObservableList<OperacaoFinanceiro> listaRegistros = null;
	...
	private String fileNameConfigColum = "Grid-Cad-OperacaoFinanceiro";

	// OBJETOS INSTACIADOS NA CLASSE
	private OperacaoFinanceiro entidadeBean = new OperacaoFinanceiro();
	private OperacaoFinanceiroDAO entidadeDao = new OperacaoFinanceiroDAO();
	...
	void actionBtnCancel(ActionEvent event) {
		...
		Util.setDefaultStyle(txtCodigo, txtDescricao, txtObservacao);
		Util.limpar(txtCodigo, txtDescricao, txtObservacao);
		tgbFlagAutorizacao.setSelected(false);
		cboxTipoOperacao.getSelectionModel().selectFirst();
	...
	void actionBtnConfig(ActionEvent event) {
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewConfigColumns.fxml",
					new ConfigColumnController(OperacaoFinanceiroController.class, tbView, fileNameConfigColum)));
			stg.setScene(scene);
			stg.initStyle(StageStyle.TRANSPARENT);
			stg.initModality(Modality.APPLICATION_MODAL);
			stg.show();
		} catch (IOException e) {
			util.tratamentoExcecao(e.getMessage().toString(), "[ OperacaoFinanceiroController.actionBtnConfig() ]");
		}
		...
		if ((entidadeBean.getFlagAtivo().equals(1) && nivAcessoPermissao.getFlagDisable().equals(1))
			...
			if (util.showAlert(
					DadosGlobais.resourceBundle.getString("alertConfirmOprExcluir") + " "
							...
							+ " " + DadosGlobais.resourceBundle.getString("operacaoFinanceiroController.alertConfirmOprExcluir"),
					"confirmation")) {
				...
				Task<String> tarefaCargaPg = new Task<String>() {
					...
					protected void failed() {
						...
						util.tratamentoExcecao(exceptionProperty().getValue().toString(),
								"[ OperacaoFinanceiroController.actionBtnDelete() ]");
						pBar.setProgress(0);
					}
				...
				t.setDaemon(true);
				t.start();
				stg = ProgressBarForm.showProgressBar(OperacaoFinanceiroController.class, tarefaCargaPg,
	...
	void actionBtnInsert(ActionEvent event) {
		...
		if (nivAcessoPermissao.getFlagInsert().equals(1)) {
			entidadeBean = new OperacaoFinanceiro();
			btnInsert.setDisable(true);
			btnSave.setDisable(false);
			btnCancel.setDisable(false);
			txtCodigo.setDisable(true);
			txtCodigo.setText("+1");
			Util.limpar(txtDescricao, txtObservacao);
			cboxTipoOperacao.getSelectionModel().selectFirst();
			tgbFlagAutorizacao.setSelected(false);
	...
	void actionBtnSave(ActionEvent event) {
		...
		if (nivAcessoPermissao.getFlagUpdate().equals(1)) {
			...
			if (!Util.noEmpty( lblErrorMessage, anchorPaneContent, txtDescricao)) {

				entidadeBean.setDescricao(Util.textfieldNotNull("str", txtDescricao));
				entidadeBean.setTipoOperacao(((EnumTipoOperacaoFinaceiro) cboxTipoOperacao.getSelectionModel()
						.getSelectedItem().getEnumOpcao()).index);
				entidadeBean.setObservacao(Util.textfieldNotNull("str", txtObservacao));
				entidadeBean.setFlagSolicitaAutorizacao(tgbFlagAutorizacao.isSelected() ? 1 : 0);
				...
				if (txtCodigo.isDisable() && txtCodigo.getText().equals("+1")) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ OperacaoFinanceiroController.actionBtnSave() - INSERT ]");
							pBar.setProgress(0);
						}
					...
					t.setDaemon(true);
					t.start();
					stg = ProgressBarForm.showProgressBar(OperacaoFinanceiroController.class, TarefaRefresh,
				...
				if (entidadeBean.getCodigo().toString().equals(txtCodigo.getText())) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ OperacaoFinanceiroController.actionBtnSave() - UPDATE ]");
							pBar.setProgress(0);
						}
	...
	public void taskQuery(String tipoConsulta, boolean flagRefresh) {
		...
		Task<String> TarefaRefresh = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ OperacaoFinanceiroController.tafefaConsulta() ]");
				pBar.setProgress(0);
			}
	...
	public void printExportShow() throws NoSuchFieldException, SecurityException {
		...
		for (TableColumn column : tbView.getVisibleLeafColumns()) {
			...
			Object type = OperacaoFinanceiro.class
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewPrintExport.fxml",
					new PrintExportController(tbView, tableShowPrintList,
							DadosGlobais.resourceBundle.getString("operacaoFinanceiroController.lblTitle"), pBar, stg)));
			...
			util.tratamentoExcecao(e.getMessage().toString(), "[ OperacaoFinanceiroController.printExportShow() ]");
		}
	...
	public void updateTbView(String flagOperacao) {
		...
		Task<String> TarefaAtualiza = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ OperacaoFinanceiroController.updateTbView() ]");
				pBar.setProgress(0);
			}
		...
		t.setDaemon(true);
		t.start();
		stg = ProgressBarForm.showProgressBar(OperacaoFinanceiroController.class, TarefaAtualiza,
		...
		if ((!flagLastRegistro && codigo > 0) || (flagLastRegistro)) {
			...
			Task<String> TarefaRefresh = new Task<String>() {
				...
				protected void succeeded() {
					...
					entidadeBean = (OperacaoFinanceiro) logRet.getObjeto();
					...
					if (entidadeBean != null) {
						...
						for (int i = 0; i < cboxTipoOperacao.getItems().size(); i++) {
							if (((EnumTipoOperacaoFinaceiro) cboxTipoOperacao.getItems().get(i).getEnumOpcao()).index
						...
						tgbFlagAutorizacao.setSelected(entidadeBean.getFlagSolicitaAutorizacao() == 1 ? true : false);
					...
					else 
					{
						...
						btnInsert.setDisable(false);
						txtCodigo.requestFocus();
						Util.limpar(txtDescricao, txtCodigo, txtObservacao);
						cboxTipoOperacao.getSelectionModel().selectFirst();
						tgbFlagAutorizacao.setSelected(false);
					}
				...
				protected void failed() {
					stg.close();
					util.tratamentoExcecao(exceptionProperty().getValue().toString(),
							"[ OperacaoFinanceiroController.loadByID() ]");
					pBar.setProgress(0);
				}
	...
	public void fillCboxTipoOperacao() {
		...
		for (EnumTipoOperacaoFinaceiro item : EnumTipoOperacaoFinaceiro.values())
	 ...
	 */
	public void updateTableView(ObservableList visibleColumns, TableView<OperacaoFinanceiro> table) {

		if (!visibleColumns.isEmpty()) {
			// hide all columns
			for (TableColumn<OperacaoFinanceiro, ?> col : table.getColumns())
				col.setVisible(false);
			// show column and defines its position
			for (int i = 0; i < visibleColumns.size(); i++) {
				table.getColumns().remove(visibleColumns.get(i));
				table.getColumns().add(i, (TableColumn<OperacaoFinanceiro, ?>) visibleColumns.get(i));
			...
			for (int i = 0; i < userDefinedColumns.size(); i++) {
				for (int j = 0; j < tbView.getColumns().size(); j++) {
					if (tbView.getColumns().get(j).getId().equals(userDefinedColumns.get(i).getName())) {
						TableColumn<OperacaoFinanceiro, ?> tableColumn = tbView.getColumns().get(j);
	...
	public void translations() {
		...
		lbTitleFormCad.setText(DadosGlobais.resourceBundle.getString("lblTitleFormCad") + " "
				+ DadosGlobais.resourceBundle.getString("miCadOperacaoFinanceiro"));
		...
		lblTipoOperacao.setText(DadosGlobais.resourceBundle.getString("operacaoFinanceiroController.lblTipoOperacao"));
		lblFlagAutorizacao.setText(DadosGlobais.resourceBundle.getString("operacaoFinanceiroController.lblFlagAutorizacao"));
		lblObservacao.setText(DadosGlobais.resourceBundle.getString("operacaoFinanceiroController.lblObservacao"));
		...
		tbColObservacao.setText(DadosGlobais.resourceBundle.getString("operacaoFinanceiroController.lblObservacao"));

		tbView.setPlaceholder(new Label(DadosGlobais.resourceBundle.getString("tbView")));

	}

	public OperacaoFinanceiroController() {
		// TODO Auto-generated constructor stub
	}

	public OperacaoFinanceiroController(NivelAcesso nivAcesso, TabPane o_tabPrincipal) {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		Util.setKeyPressDefaultStyles(txtCodigo, txtDescricao, txtObservacao);
		Util.setStyleOnFocus(txtFilterColumn,txtCodigo, txtDescricao, txtObservacao);

		Util.onlyNumbers(txtCodigo);

		Util.onlyAlphanumeric(txtDescricao, txtFilterColumn, txtObservacao);
		...
		Util.whriteUppercase(txtFilterColumn, txtDescricao, txtObservacao);
		...
		tbColCodigo.setCellValueFactory(new PropertyValueFactory<OperacaoFinanceiro, Integer>("codigo"));
		tbColDescricao.setCellValueFactory(new PropertyValueFactory<OperacaoFinanceiro, String>("descricao"));
		tbColAtivoInat.setCellValueFactory(new PropertyValueFactory<OperacaoFinanceiro, Integer>("flagAtivo"));
		tbColCodeEmp.setCellValueFactory(new PropertyValueFactory<OperacaoFinanceiro, Integer>("codemp"));
		tbColObservacao.setCellValueFactory(new PropertyValueFactory<OperacaoFinanceiro, String>("observacao"));
		...
		tbView.setRowFactory(tb -> {
			...
			// TENHA O FLAGATIVO = 0
			TableRow<OperacaoFinanceiro> row = new TableRow<OperacaoFinanceiro>() {
				@Override
				public void updateItem(OperacaoFinanceiro objeto, boolean empty) {
