...
import models.configuracoes.LayoutArquivoDet;
import models.configuracoes.LayoutArquivoDetPK;
...
import tools.enums.EnumCboxSeparador;
import tools.enums.EnumCboxTipoArquivo;
import tools.enums.EnumCboxTipoOperacao;
import tools.enums.EnumDefaultFields;
...
public class LayoutsArquivoController implements Initializable 
{
	...
	@FXML
	private ComboBox<ComboBoxFilter> cboxTipoArquivo,
									 cboxTipoOperacao,
									 cboxSeparator,
									 cboxTabelas,
									 cboxFilterColumn,
	...
	@FXML
	public TableView<LayoutArquivoCab> tbView;
	
	@FXML
	public TableView <TabelasModel> tvSelectedTabelaFields;

	@FXML
	private TableColumn<LayoutArquivoCab, Integer> tbColCodigo, tbColCodeEmp, tbColAtivoInat ;

	@FXML
	private TableColumn<LayoutArquivoCab, String> tbColDescricao;

	@FXML
	private TextField txtCodigo, txtDescricao;

	@FXML
	private CustomTextField txtSearchLayout, txtFilterColumn,
	txtFileName;
	...
	@FXML
	private Label lblCboxFilterColumn, lblcboxFlagAtivo, lblDetalhes, lblDescriao, lblDetCodigo, lblDetDesc, lbTitleFormCadTabelasAuxiliares;
	// ------------------- ELEMENTOS CONTIDOS NO FORMULARIO FXML ------------------------

	// ATRIBUTOS DA CLASSE
	private ObservableList<LayoutArquivoCab> listaRegistros = null;
	...
	private LayoutArquivoCab entidadeBean = new LayoutArquivoCab();
	private LayoutArquivoCabDAO entidadeDao = new LayoutArquivoCabDAO();
	private Util util = new Util();
	private PopOver popOver = new PopOver();
	private static Stage stg;
	private String fileNameConfigColum = "Grid-Cad-Grupo";
	
	public ObservableList<ComboBoxFilter> listCboxTipoArquivo = FXCollections.observableArrayList(),
			  							  listCboxTipoOperacao = FXCollections.observableArrayList(),
			  							  listCboxSeparator = FXCollections.observableArrayList(),
			  							  listCboxTabelas = FXCollections.observableArrayList();

	// CONSTRUTOR PADRAO DA CLASSE
	public LayoutsArquivoController() 
	{
	...
	// FORMULARIO
	public LayoutsArquivoController(NivelAcesso nivAcesso, TabPane o_tabPrincipal)
	{
	...
	void actionBtnInsertGrid(ActionEvent event) 
	{
		...
		{
			entidadeBean = new LayoutArquivoCab();
	...
	void actionBtnSave(ActionEvent event) 
	{
		...
		if ( nivAcessoPermissao.getFlagUpdate().equals(1)) 
		{
			...
			{							
			    ...
			    entidadeBean.setTipoArquivoDados(cboxTipoArquivo.getSelectionModel().getSelectedIndex());
			    entidadeBean.setFlagTipoOperacao(cboxTipoOperacao.getSelectionModel().getSelectedIndex());
			    entidadeBean.setSeparador(cboxSeparator.getSelectionModel().getSelectedItem().getValue());
			    entidadeBean.setTabela(cboxTabelas.getSelectionModel().getSelectedItem().getValue());	
			    entidadeBean.setComandoSql("SELECT * FROM layout_arquivo_cab;");	        		
				...
				if ( txtCodigo.isDisable() && txtCodigo.getText().equals("+1") ) 
				{
					...
					Task<String> TarefaRefresh = new Task<String>() 
					{
						...
						{	   					    
							logRet = entidadeDao.insert(entidadeBean, tvSelectedTabelaFields);
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),"[ GrupoController.actionBtnSave() - INSERT ]");
							pBar.setProgress(0);
						}
				...
				else if ( entidadeBean.getCodigo().toString().equals(txtCodigo.getText()) ) 
				{
						...
						Task<String> TarefaRefresh = new Task<String>() 
						{
							...
							protected String call() throws Exception 
							{
								...
								logRet = entidadeDao.update(entidadeBean, tvSelectedTabelaFields);
							...
							protected void failed() 
							{
								...
								util.tratamentoExcecao(exceptionProperty().getValue().toString(),
										"[ GrupoController.actionBtnSave() - UPDATE ]");
								pBar.setProgress(0);
							}
	...
	void actionBtnConfig(ActionEvent event) 
	{
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewConfigColumns.fxml",
					new ConfigColumnController(TabelaAuxiliarCfopController.class, tbView, fileNameConfigColum)));
			stg.setScene(scene);
			stg.initStyle(StageStyle.TRANSPARENT);
			stg.initModality(Modality.APPLICATION_MODAL);
			stg.show();
		} catch (IOException e) {
			util.tratamentoExcecao(e.getMessage().toString(), "[ GrupoController.actionBtnConfig() ]");
		}
	...
	void actionBtnDelete(ActionEvent event) {
		...
		if ((entidadeBean.getFlagAtivo()==0 && nivAcessoPermissao.getFlagDisable().equals(1))
				|| (entidadeBean.getFlagAtivo()==0 && nivAcessoPermissao.getFlagEnable().equals(1))) {

			if (util.showAlert(DadosGlobais.resourceBundle.getString("alertConfirmOprExcluir") + " "
					+ (entidadeBean.getFlagAtivo()==1 ? DadosGlobais.resourceBundle.getString("oprExcluir")
							: DadosGlobais.resourceBundle.getString("oprAtivar"))
					+ " " + DadosGlobais.resourceBundle.getString("alertConfirmOprExcluirGrupo"), "confirmation")) {

				entidadeBean.setFlagAtivo(entidadeBean.getFlagAtivo()==1 ? 0 : 1);
				...
				Task<String> tarefaCargaPg = new Task<String>() {
					...
					protected void failed() {
						stg.close();
						util.tratamentoExcecao(exceptionProperty().getValue().toString(),
								"[ GrupoController.actionBtnDelete() ]");
						pBar.setProgress(0);
					}
				...
				t.setDaemon(true);
				t.start();
				stg = ProgressBarForm.showProgressBar(TabelaAuxiliarCfopController.class, tarefaCargaPg,
						tbView.getSelectionModel().getSelectedItem().getFlagAtivo() == 1
			...
			Task<String> TarefaRefresh = new Task<String>() 
			{
				...
				protected String call() throws Exception 
				{
					...
					if (!flagLastRegistro) 
					{
						...
						logRet = entidadeDao.getById(codigo);
				...
				protected void succeeded() 
				{
					...
					entidadeBean = (LayoutArquivoCab) logRet.getObjeto();
					...
					if (entidadeBean != null) 
					{
						...
						String separador = entidadeBean.getSeparador(),
							   tabelas = entidadeBean.getTabela(); 
						
						cboxTipoArquivo.getSelectionModel().select(entidadeBean.getTipoArquivoDados());
						cboxTipoOperacao.getSelectionModel().select(entidadeBean.getFlagTipoOperacao());
						cboxSeparator.setValue(new ComboBoxFilter(separador, 0, separador));
						cboxTabelas.setValue(new ComboBoxFilter(tabelas, 0, tabelas));
						
					    List<LayoutArquivoDet> listLayoutArquivoDet = entidadeBean.getLayoutArquivoDet();
						...
						for ( int l = 0 ; l < listLayoutArquivoDet.size() ; l++ )
						{	
							  LayoutArquivoDet lad = listLayoutArquivoDet.get(l);
							  
							  String descColumn = lad.getDescColuna() ,
							         dataType = lad.getTipoDados(),
							         filPos = lad.getPosicaoFile();
					    		...
					    		{	
					        		  ...
					        		  if( descColumn.equalsIgnoreCase(fieldName) && dataType.equalsIgnoreCase(fieldType))
					        		  {	
	...
	public void taskQuery(String tipoConsulta, boolean flagRefresh) 
	{
		...
		Task<String> TarefaRefresh = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ GrupoController.tafefaConsulta() ]");
				pBar.setProgress(0);
			}
	...
	public void setIdioma() {
		...
		//lbTitleFormCadTabelasAuxiliares.setText("Tabelas Auxiliares");
	 ...
	 */
	public void updateTableView(ObservableList visibleColumns, TableView<GrupoDAO> table) {

		if (!visibleColumns.isEmpty()) {
			// hide all columns
			for (TableColumn<GrupoDAO, ?> col : table.getColumns())
				col.setVisible(false);
			// show column and defines its position
			for (int i = 0; i < visibleColumns.size(); i++) {
				table.getColumns().remove(visibleColumns.get(i));
				table.getColumns().add(i, (TableColumn<GrupoDAO, ?>) visibleColumns.get(i));
		...
		if (userDefinedColumns != null) 
		{
			...
			for (int i = 0; i < userDefinedColumns.size(); i++) 
			{
				for (int j = 0; j < tbView.getColumns().size(); j++) 
				{
					...
					{
						TableColumn<LayoutArquivoCab, ?> tableColumn = tbView.getColumns().get(j);
	...
	public void printExportShow() throws NoSuchFieldException, SecurityException {
		...
		for (TableColumn column : tbView.getVisibleLeafColumns()) {
			...
			Object type = Grupo.class.getDeclaredField(((PropertyValueFactory) column.getCellValueFactory()).getProperty()).getType().getSimpleName();
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewPrintExport.fxml",
					new PrintExportController(tbView, tableShowPrintList, "Grupo", pBar, stg)));
			...
			util.tratamentoExcecao(e.getMessage().toString(), "[ GrupoController.printExportShow() ]");
		}
	...
	/**
	 * Fill comboBox fillCboxTipoArquivo and property definitions
	 */
	public void fillCboxTipoArquivo() 
	{		
                 
				for ( EnumCboxTipoArquivo item : EnumCboxTipoArquivo.values() )
					  listCboxTipoArquivo.add(new ComboBoxFilter(item.text, 0, item.index.toString()));
		
				cboxTipoArquivo.setItems(listCboxTipoArquivo);
				cboxTipoArquivo.getSelectionModel().clearAndSelect(0);
				cboxTipoArquivo.setValue(cboxTipoArquivo.getSelectionModel().getSelectedItem());
				
				cboxTipoArquivo.setConverter(new StringConverter<ComboBoxFilter>() 
				{
	 ...
	 * Fill comboBox fillCboxTipoArquivo and property definitions
	 */
	public void fillCboxTipoOperacao() 
	{		
                 
				for ( EnumCboxTipoOperacao item : EnumCboxTipoOperacao.values() )
					  listCboxTipoOperacao.add(new ComboBoxFilter(item.text, 0, item.index.toString()));
		
				cboxTipoOperacao.setItems(listCboxTipoOperacao);
				cboxTipoOperacao.getSelectionModel().clearAndSelect(0);
				cboxTipoOperacao.setValue(cboxTipoOperacao.getSelectionModel().getSelectedItem());
	 ...
	 * Fill comboBox fillCboxTipoArquivo and property definitions
	 */
	public void fillCboxSeparator() 
	{		
                 
			for ( EnumCboxSeparador item : EnumCboxSeparador.values() )
				  listCboxSeparator.add(new ComboBoxFilter(item.text, 0, item.index.toString()));
	
			cboxSeparator.setItems(listCboxSeparator);
			cboxSeparator.getSelectionModel().clearAndSelect(0);
			cboxSeparator.setValue(cboxSeparator.getSelectionModel().getSelectedItem());
			
			cboxSeparator.setConverter(new StringConverter<ComboBoxFilter>() 
			{
	...
	/**
	 * Fill comboBox fillCboxTabelas and property definitions
	 */
	public void fillCboxTabelas() 
	{	        	
        	listCboxTabelas.clear();
            ...
            {		            	
            	...
            	Class<?> javaClass = et.getJavaType();	
            	String simpleName = javaClass.getSimpleName(); 	            	
            	listCboxTabelas.add(new ComboBoxFilter(simpleName, 0, simpleName));
            }	
            
            cboxTabelas.setItems(listCboxTabelas);
            cboxTabelas.getSelectionModel().clearAndSelect(0);
            cboxTabelas.setValue(cboxTabelas.getSelectionModel().getSelectedItem());
            changeCboxTabelas();	
            
            cboxTabelas.setConverter(new StringConverter<ComboBoxFilter>() 
			{
	...
	public ObservableList <Field> getFilteredDeclaredFields( Field[] declaredFields)
	{
		    ObservableList <Field> filteredFieldList = FXCollections.observableArrayList();
			...
			for ( int i = 0; i < declaredFields.length; i++ )
			{
				  ...
				  if( !EnumDefaultFields.Contains(fieldName) )	  
	...
	{	
            ...
            {		            	
	            	...
	            	if( cboxTabelas.getSelectionModel().getSelectedItem().getValue().equalsIgnoreCase(simpleName) )
	            	{	
		        		    tvSelectedTabelaFields.getItems().clear();
		        		    tvSelectedTabelaFields.getColumns().clear(); 
		        		    
							TableColumn fieldChkCol = new TableColumn(""),
										fieldNameCol = new TableColumn("Field Name"),
									    fieldTypeCol = new TableColumn("Field Type"),
									    fieldValCol = new TableColumn("Posição no arquivo");	
							
		        	        tvSelectedTabelaFields.getColumns().addAll(fieldChkCol, fieldNameCol, fieldTypeCol, fieldValCol);	
		        	        
		        	        ObservableList<TabelasModel> tabelaItems = FXCollections.observableArrayList();
		        	        
		        	        fieldChkCol.setCellValueFactory( new PropertyValueFactory<>("fieldChk") );
		        	        ...
		        	        fieldValCol.setCellValueFactory( new PropertyValueFactory<>("fieldVal") );		        		    
		       
		            		Field[] decalredFields = javaClass.getDeclaredFields();
		            		ObservableList <Field> filteredFieldList = getFilteredDeclaredFields(decalredFields);		            		
		            		
		            		for ( int f = 0; f < filteredFieldList.size(); f++ ) 
		            		{
		            			  Field field = filteredFieldList.get(f);
		            			  String fieldName = field.getName(),
		            			         fieldType = field.getType().getSimpleName();
		            			  
		            			  tabelaItems.add(new TabelasModel("", fieldName, fieldType, ""));
							}		 
		            		
		            		tvSelectedTabelaFields.setItems(tabelaItems);			            		
            ...
            initChkBoxAndTextFieldTabelasState();			
	}
	...
	public void initChkBoxAndTextFieldTabelasState() 
	{	
			ObservableList<TableColumn<TabelasModel,?>> cols = tvSelectedTabelaFields.getColumns();
			int rowCount = tvSelectedTabelaFields.getItems().size();		
			for ( int f = 0 ; f < rowCount ; f++ ) 
	...
	public void initialize(URL location, ResourceBundle resources) 
	{
		...
		cboxTabelas.setOnAction(new EventHandler<ActionEvent>() 
		{			
			@Override
		...
		tbColCodigo.setCellValueFactory(new PropertyValueFactory<LayoutArquivoCab, Integer>("codigo"));
		tbColDescricao.setCellValueFactory(new PropertyValueFactory<LayoutArquivoCab, String>("descricao"));
		tbColCodeEmp.setCellValueFactory(new PropertyValueFactory<LayoutArquivoCab, Integer>("codemp"));
		tbColAtivoInat.setCellValueFactory(new PropertyValueFactory<LayoutArquivoCab, Integer>("flagAtivo"));
		...
		tbView.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
			if (event.getCode().equals(KeyCode.ENTER) && tbView.getSelectionModel().getSelectedItem().getFlagAtivo() == 1) 
			{
		...
		tbView.setRowFactory(tb -> {
			...
			// TENHA O FLAGATIVO = 0
			TableRow<LayoutArquivoCab> row = new TableRow<LayoutArquivoCab>() 
			{
				@Override
				public void updateItem(LayoutArquivoCab objeto, boolean empty) 
				{
						super.updateItem(objeto, empty);
						
						if (objeto == null)
							setStyle("");
						else if (objeto.getFlagAtivo()==0) 
			...
			row.setOnMouseClicked(tr -> {
				...
				if (tr.getButton().equals(MouseButton.SECONDARY)) {
					...
					itemAtualizar.setOnAction(new EventHandler<ActionEvent>() 
					{
						...
						public void handle(ActionEvent event) 
						{
							txtCodigo.setText( tbView.getSelectionModel().getSelectedItem().getCodigo().toString());
							loadByID(false,tbView.getSelectionModel().getSelectedItem().getCodigo(), false);
							Util.openFormCadastro(anchorPaneListagem, anchorPaneDetalhes, 0);
						}
					});
						...
						{	
							try 
							{
								util.crearTab(tabPrincipal, DadosGlobais.resourceBundle.getString("miCadItem") , "/views/compras/viewItens.fxml", 
										new ItemController(Util.getNivelAcessoEntidade(EnumNivelAcesso.ITENS), "Cod_Grupo", tbView.getSelectionModel().getSelectedItem()/*.getCodigo()*/.toString() ), false);
							} 
							catch (IOException e1) 
							{
								util.tratamentoExcecao(e1.getMessage().toString(),"[ GrupoController.acaoRastrearItens() ]");
							}
						}
					});
		...
		tbColAtivoInat.setCellFactory(col -> new TableCell<LayoutArquivoCab, Integer>() 
		{
			@Override
