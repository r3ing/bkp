...
public class RegiaoController implements Initializable {
	...
	@FXML
	private TableView<Regiao> tbView;

	@FXML
	private TableColumn<Regiao, Integer> tbColCodigo;

	@FXML
	private TableColumn<Regiao, String> tbColDescricao;

	@FXML
	private TableColumn<Regiao, Integer> tbColAtivoInat;
	...
	// ATRIBUTOS DA CLASSE
	private ObservableList<Regiao> listaRegistros = null;
	...
	private Regiao entidadeBean = new Regiao();
	private RegiaoDAO entidadeDao = new RegiaoDAO();
	...
	void actionBtnConfig(ActionEvent event) {
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewConfigColumns.fxml",
					new ConfigColumnController(RegiaoController.class, tbView, "Grid-Cad-Regiao")));
			stg.setScene(scene);
			stg.initStyle(StageStyle.TRANSPARENT);
			stg.initModality(Modality.APPLICATION_MODAL);
			stg.show();
		} catch (IOException e) {
			util.tratamentoExcecao(e.getMessage().toString(), "[ RegiaoController.actionBtnConfig() ]");
		}
		...
		if ((entidadeBean.getFlagAtivo().equals(1) && nivAcessoPermissao.getFlagDisable().equals(1))
			...
			if (util.showAlert(
					DadosGlobais.resourceBundle.getString("alertConfirmOprExcluir") + " "
							...
							+ " " + DadosGlobais.resourceBundle.getString("regiaoController.alertConfirmOprExcluir"),
					"confirmation")) {
				...
				Task<String> tarefaCargaPg = new Task<String>() {
					...
					protected void failed() {
						...
						util.tratamentoExcecao(exceptionProperty().getValue().toString(),
								"[ RegiaoController.actionBtnDelete() ]");
						pBar.setProgress(0);
					}
				...
				t.setDaemon(true);
				t.start();
				stg = ProgressBarForm.showProgressBar(RegiaoController.class, tarefaCargaPg,
				...
				if (txtCodigo.isDisable() && txtCodigo.getText().equals("+1")) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ RegiaoController.actionBtnSave() - INSERT ]");
							pBar.setProgress(0);
						}
					...
					t.setDaemon(true);
					t.start();
					stg = ProgressBarForm.showProgressBar(RegiaoController.class, TarefaRefresh,
				...
				if (entidadeBean.getCodigo().toString().equals(txtCodigo.getText())) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ RegiaoController.actionBtnSave() - UPDATE ]");
							pBar.setProgress(0);
						}
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewPrintExport.fxml",
					new PrintExportController(tbView, tableShowPrintList,
							DadosGlobais.resourceBundle.getString("regiaoController.lblTitle"), pBar, stg)));
			...
			util.tratamentoExcecao(e.getMessage().toString(), "[ RegiaoController.printExportShow() ]");
		}
	...
	public void taskQuery(String tipoConsulta, boolean flagRefresh) {
		...
		Task<String> TarefaRefresh = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ RegiaoController.tafefaConsulta() ]");
				pBar.setProgress(0);
			}
	...
	public void updateTbView(String flagOperacao) {
		...
		t.setDaemon(true);
		t.start();
		stg = ProgressBarForm.showProgressBar(RegiaoController.class, TarefaAtualiza,
		...
		if ((!flagLastRegistro && codigo > 0) || (flagLastRegistro)) {
			...
			Task<String> TarefaRefresh = new Task<String>() {
				...
				protected void succeeded() {
					...
					entidadeBean = (Regiao) logRet.getObjeto();
	 ...
	 */
	public void updateTableView(ObservableList visibleColumns, TableView<Regiao> table) {

		if (!visibleColumns.isEmpty()) {
			// hide all columns
			for (TableColumn<Regiao, ?> col : table.getColumns())
				col.setVisible(false);
			// show column and defines its position
			for (int i = 0; i < visibleColumns.size(); i++) {
				table.getColumns().remove(visibleColumns.get(i));
				table.getColumns().add(i, (TableColumn<Regiao, ?>) visibleColumns.get(i));
	...
	public void saveColumnSettings() {
		...
		xml.WriteXMLColumns(userDefinedColumnList, "Grid-Cad-Regiao");
	...
	public void configTableView() {
		...
		ArrayList<UserDefinedColumn> userDefinedColumns = xmlTableColumns.ReadXMLColumns("Grid-Cad-Regiao");
			...
			for (int i = 0; i < userDefinedColumns.size(); i++) {
				for (int j = 0; j < tbView.getColumns().size(); j++) {
					if (tbView.getColumns().get(j).getId().equals(userDefinedColumns.get(i).getName())) {
						TableColumn<Regiao, ?> tableColumn = tbView.getColumns().get(j);
	...
	public void translations() {
		...
		lbTitleFormCad.setText(DadosGlobais.resourceBundle.getString("lblTitleFormCad") + " "
				+ DadosGlobais.resourceBundle.getString("miCadRegioesClientes"));
	...
	public RegiaoController(NivelAcesso nivAcesso, TabPane o_tabPrincipal) {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		tbColCodigo.setCellValueFactory(new PropertyValueFactory<Regiao, Integer>("codigo"));
		tbColDescricao.setCellValueFactory(new PropertyValueFactory<Regiao, String>("descricao"));
		tbColAtivoInat.setCellValueFactory(new PropertyValueFactory<Regiao, Integer>("flagAtivo"));
		...
		tbView.setRowFactory(tb -> {
			...
			// TENHA O FLAGATIVO = 0
			TableRow<Regiao> row = new TableRow<Regiao>() {
				@Override
				public void updateItem(Regiao objeto, boolean empty) {
						...
						public void handle(ActionEvent e) {
							try {
								...
								util.tratamentoExcecao(e1.getMessage().toString(),
										"[ RegiaoController.acaoRastrearItens() ]");
							}
						}
					});
