...
import models.vendas.MensagemNotaFiscal;
import models.vendas.MensagemNotaFiscalDAO;
...
public class MensagemController implements Initializable {
	...
	@FXML
	private TableView<MensagemNotaFiscal> tbView;

	@FXML
	private TableColumn<MensagemNotaFiscal, Integer> tbColCodigo;

	@FXML
	private TableColumn<MensagemNotaFiscal, String> tbColDescricao;

	@FXML
	private TableColumn<MensagemNotaFiscal, String> tbColMensagem;

	@FXML
	private TableColumn<MensagemNotaFiscal, Integer> tbColAtivoInat;
	...
	// ATRIBUTOS DA CLASSE
	private ObservableList<MensagemNotaFiscal> listaRegistros = null;
	...
	private MensagemNotaFiscal entidadeBean = new MensagemNotaFiscal();
	private MensagemNotaFiscalDAO entidadeDao = new MensagemNotaFiscalDAO();
	...
	void actionBtnConfig(ActionEvent event) {
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewConfigColumns.fxml",
					new ConfigColumnController(MensagemController.class, tbView, "Grid-Cad-MensagemNotaFiscal")));
			stg.setScene(scene);
			stg.initStyle(StageStyle.TRANSPARENT);
			stg.initModality(Modality.APPLICATION_MODAL);
			stg.show();
		} catch (IOException e) {
			util.tratamentoExcecao(e.getMessage().toString(), "[ MensagemController.actionBtnConfig() ]");
		}
		...
		if ((entidadeBean.getFlagAtivo().equals(1) && nivAcessoPermissao.getFlagDisable().equals(1))
			...
			if (util.showAlert(
					DadosGlobais.resourceBundle.getString("alertConfirmOprExcluir") + " "
							...
							+ " " + DadosGlobais.resourceBundle.getString("mensagemController.alertConfirmOprExcluir"),
					"confirmation")) {
				...
				Task<String> tarefaCargaPg = new Task<String>() {
					...
					protected void failed() {
						...
						util.tratamentoExcecao(exceptionProperty().getValue().toString(),
								"[ MensagemController.actionBtnDelete() ]");
						pBar.setProgress(0);
					}
				...
				t.setDaemon(true);
				t.start();
				stg = ProgressBarForm.showProgressBar(MensagemController.class, tarefaCargaPg,
	...
	void actionBtnInsert(ActionEvent event) {
		...
		if (nivAcessoPermissao.getFlagInsert().equals(1)) {
			entidadeBean = new MensagemNotaFiscal();
	...
	void actionBtnSave(ActionEvent event) {
		...
		if (nivAcessoPermissao.getFlagUpdate().equals(1)) {
			...
			if (!Util.noEmpty(lblErrorMessage, anchorPaneDetalhes, txtDescricao)) {
				...
				entidadeBean.setObservacao(Util.textfieldNotNull("str", txtMensagem));
				...
				if (txtCodigo.isDisable() && txtCodigo.getText().equals("+1")) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ MensagemController.actionBtnSave() - INSERT ]");
							pBar.setProgress(0);
						}
					...
					t.setDaemon(true);
					t.start();
					stg = ProgressBarForm.showProgressBar(MensagemController.class, TarefaRefresh,
				...
				if (entidadeBean.getCodigo().toString().equals(txtCodigo.getText())) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ MensagemController.actionBtnSave() - UPDATE ]");
							pBar.setProgress(0);
						}
	...
	public void taskQuery(String tipoConsulta, boolean flagRefresh) {
		...
		Task<String> TarefaRefresh = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ MensagemController.tafefaConsulta() ]");
				pBar.setProgress(0);
			}
	...
	public void printExportShow() throws NoSuchFieldException, SecurityException {
		...
		for (TableColumn column : tbView.getVisibleLeafColumns()) {
			...
			Object type = MensagemNotaFiscal.class
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewPrintExport.fxml",
					new PrintExportController(tbView, tableShowPrintList,
							DadosGlobais.resourceBundle.getString("mensagemController.lblTitle"), pBar, stg)));
			...
			util.tratamentoExcecao(e.getMessage().toString(), "[ MensagemController.printExportShow() ]");
		}
	}
	...
	public void updateTbView(String flagOperacao) {
		...
		Task<String> TarefaAtualiza = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ MensagemController.updateTbView() ]");
				pBar.setProgress(0);
			}
		...
		t.setDaemon(true);
		t.start();
		stg = ProgressBarForm.showProgressBar(MensagemController.class, TarefaAtualiza,
		...
		if ((!flagLastRegistro && codigo > 0) || (flagLastRegistro)) {
			...
			Task<String> TarefaRefresh = new Task<String>() {
				...
				protected void succeeded() {
					...
					entidadeBean = (MensagemNotaFiscal) logRet.getObjeto();
					...
					if (entidadeBean != null) {
						...
						txtMensagem.setText(entidadeBean.getObservacao());
	 ...
	 */
	public void updateTableView(ObservableList visibleColumns, TableView<MensagemNotaFiscal> table) {

		if (!visibleColumns.isEmpty()) {
			// hide all columns
			for (TableColumn<MensagemNotaFiscal, ?> col : table.getColumns())
				col.setVisible(false);
			// show column and defines its position
			for (int i = 0; i < visibleColumns.size(); i++) {
				table.getColumns().remove(visibleColumns.get(i));
				table.getColumns().add(i, (TableColumn<MensagemNotaFiscal, ?>) visibleColumns.get(i));
	...
	public void saveColumnSettings() {
		...
		xml.WriteXMLColumns(userDefinedColumnList, "Grid-Cad-MensagemNotaFiscal");
	...
	public void configTableView() {
		...
		ArrayList<UserDefinedColumn> userDefinedColumns = xmlTableColumns.ReadXMLColumns("Grid-Cad-MensagemNotaFiscal");
			...
			for (int i = 0; i < userDefinedColumns.size(); i++) {
				for (int j = 0; j < tbView.getColumns().size(); j++) {
					if (tbView.getColumns().get(j).getId().equals(userDefinedColumns.get(i).getName())) {
						TableColumn<MensagemNotaFiscal, ?> tableColumn = tbView.getColumns().get(j);
	...
	public void translations() {
		...
		lbTitleFormCad.setText(DadosGlobais.resourceBundle.getString("lblTitleFormCad") + " "
				+ DadosGlobais.resourceBundle.getString("miCadMensagem"));
		...
		lblMensagem.setText(DadosGlobais.resourceBundle.getString("mensagemController.lblMensagem"));
		...
		tbColMensagem.setText(DadosGlobais.resourceBundle.getString("mensagemController.lblMensagem"));
	...
	public MensagemController(NivelAcesso nivAcesso, TabPane o_tabPrincipal) {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		Util.whriteUppercase(txtMensagem);
		...
		tbColCodigo.setCellValueFactory(new PropertyValueFactory<MensagemNotaFiscal, Integer>("codigo"));
		tbColDescricao.setCellValueFactory(new PropertyValueFactory<MensagemNotaFiscal, String>("descricao"));
		tbColMensagem.setCellValueFactory(new PropertyValueFactory<MensagemNotaFiscal, String>("observacao"));
		tbColAtivoInat.setCellValueFactory(new PropertyValueFactory<MensagemNotaFiscal, Integer>("flagAtivo"));
		...
		tbView.setRowFactory(tb -> {
			...
			// TENHA O FLAGATIVO = 0
			TableRow<MensagemNotaFiscal> row = new TableRow<MensagemNotaFiscal>() {
				@Override
				public void updateItem(MensagemNotaFiscal objeto, boolean empty) {
						...
						public void handle(ActionEvent e) {
							try {
								...
								util.tratamentoExcecao(e1.getMessage().toString(),
										"[ MensageController.acaoRastrearItens() ]");
							}
						}
					});
