...
public class ConvenioController implements Initializable {
	...
	@FXML
	private TableView<Convenio> tbView;

	@FXML
	private TableColumn<Convenio, Integer> tbColCodigo;

	@FXML
	private TableColumn<Convenio, String> tbColDescricao;

	@FXML
	private TableColumn<Convenio, Integer> tbColAtivoInat;
	...
	// ATRIBUTOS DA CLASSE
	private ObservableList<Convenio> listaRegistros = null;
	...
	private String fileNameConfigColum = "Grid-Cad-Convenio";

	// OBJETOS INSTACIADOS NA CLASSE
	private Convenio entidadeBean = new Convenio();
	private ConvenioDAO entidadeDao = new ConvenioDAO();
	...
	void actionBtnConfig(ActionEvent event) {
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewConfigColumns.fxml",
					new ConfigColumnController(ConvenioController.class, tbView, fileNameConfigColum)));
			stg.setScene(scene);
			stg.initStyle(StageStyle.TRANSPARENT);
			stg.initModality(Modality.APPLICATION_MODAL);
			stg.show();
		} catch (IOException e) {
			util.tratamentoExcecao(e.getMessage().toString(), "[ ConvenioController.actionBtnConfig() ]");
		}
		...
		if ((entidadeBean.getFlagAtivo().equals(1) && nivAcessoPermissao.getFlagDisable().equals(1))
			...
			if (util.showAlert(
					DadosGlobais.resourceBundle.getString("alertConfirmOprExcluir") + " "
							...
							+ " " + DadosGlobais.resourceBundle.getString("convenioController.alertConfirmOprExcluir"),
					"confirmation")) {
				...
				Task<String> taskDelete = new Task<String>() {
					...
					protected void failed() {
						...
						util.tratamentoExcecao(exceptionProperty().getValue().toString(),
								"[ ConvenioController.actionBtnDelete() ]");
						pBar.setProgress(0);
					}
				...
				t.setDaemon(true);
				t.start();
				stg = ProgressBarForm.showProgressBar(ConvenioController.class, taskDelete,
				...
				if (txtCodigo.isDisable() && txtCodigo.getText().equals("+1")) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ ConvenioController.actionBtnSave() - INSERT ]");
							pBar.setProgress(0);
						}
					...
					t.setDaemon(true);
					t.start();
					stg = ProgressBarForm.showProgressBar(ConvenioController.class, TarefaRefresh,
				...
				if (entidadeBean.getCodigo().toString().equals(txtCodigo.getText())) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ ConvenioController.actionBtnSave() - UPDATE ]");
							pBar.setProgress(0);
						}
	...
	public void taskQuery(String tipoConsulta, boolean flagRefresh) {
		...
		Task<String> TarefaRefresh = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ ConvenioController.tafefaConsulta() ]");
				pBar.setProgress(0);
			}
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewPrintExport.fxml",
					new PrintExportController(tbView, tableShowPrintList,
							DadosGlobais.resourceBundle.getString("convenioController.lblTitle"), pBar, stg)));
			...
			util.tratamentoExcecao(e.getMessage().toString(), "[ ConvenioController.printExportShow() ]");
		}
	...
	public void updateTbView(String flagOperacao) {
		...
		Task<String> TarefaAtualiza = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ ConvenioController.updateTbView() ]");
				pBar.setProgress(0);
			}
		...
		t.setDaemon(true);
		t.start();
		stg = ProgressBarForm.showProgressBar(ConvenioController.class, TarefaAtualiza,
		...
		if ((!flagLastRegistro && codigo > 0) || (flagLastRegistro)) {
			...
			Task<String> TarefaRefresh = new Task<String>() {
				...
				protected void succeeded() {
					...
					entidadeBean = (Convenio) logRet.getObjeto();
	 ...
	 */
	public void updateTableView(ObservableList visibleColumns, TableView<Convenio> table) {

		if (!visibleColumns.isEmpty()) {
			// hide all columns
			for (TableColumn<Convenio, ?> col : table.getColumns())
				col.setVisible(false);
			// show column and defines its position
			for (int i = 0; i < visibleColumns.size(); i++) {
				table.getColumns().remove(visibleColumns.get(i));
				table.getColumns().add(i, (TableColumn<Convenio, ?>) visibleColumns.get(i));
			...
			for (int i = 0; i < userDefinedColumns.size(); i++) {
				for (int j = 0; j < tbView.getColumns().size(); j++) {
					if (tbView.getColumns().get(j).getId().equals(userDefinedColumns.get(i).getName())) {
						TableColumn<Convenio, ?> tableColumn = tbView.getColumns().get(j);
	...
	public void translations() {
		...
		lbTitleFormCad.setText(DadosGlobais.resourceBundle.getString("lblTitleFormCad") + " "
				+ DadosGlobais.resourceBundle.getString("miCadConvenio"));
	...
	public ConvenioController(NivelAcesso nivAcesso, TabPane o_tabPrincipal) {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		tbColCodigo.setCellValueFactory(new PropertyValueFactory<Convenio, Integer>("codigo"));
		tbColDescricao.setCellValueFactory(new PropertyValueFactory<Convenio, String>("descricao"));
		tbColAtivoInat.setCellValueFactory(new PropertyValueFactory<Convenio, Integer>("flagAtivo"));
		...
		tbView.setRowFactory(tb -> {
			...
			TableRow<Convenio> row = new TableRow<Convenio>() {
				@Override
				public void updateItem(Convenio objeto, boolean empty) {
						...
						public void handle(ActionEvent e) {
							try {
								...
								util.tratamentoExcecao(e1.getMessage().toString(),
										"[ ConvenioController.acaoRastrearItens() ]");
							}
						}
					});
