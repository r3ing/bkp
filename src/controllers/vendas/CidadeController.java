...
public class CidadeController implements Initializable {
	...
	@FXML
	private TableColumn<Cidade, Integer> tbColCodigo;

	@FXML
	private TableColumn<Cidade, String> tbColDescricao;

	@FXML
	private TableColumn<Cidade, String> tbColCodIbge;

	@FXML
	private TableColumn<Cidade, String> tbColUf;

	@FXML
	private TableColumn<Cidade, Integer> tbColAtivoInat;
	...
	@FXML
	private CustomTextField txtCodUf;

	@FXML
	private CustomTextField txtUf;
	...
	// ATRIBUTOS DA CLASSE
	private ObservableList<Cidade> listaRegistros = null;
	...
	private Cidade entidadeBean = new Cidade();
	private CidadeDAO entidadeDao = new CidadeDAO();
	...
	void actionBtnClose(ActionEvent event) {
		...
		Util.setDefaultStyle(txtCodigo, txtDescricao, txtCodUf, txtUf);
	...
	void actionBtnConfig(ActionEvent event) {
		...
		try {
			...
			Scene scene = new Scene(util.openWindow("/views/utils/viewConfigColumns.fxml",
					new ConfigColumnController(CidadeController.class, tbView, "Grid-Cad-Cidade")));
			stg.setScene(scene);
			stg.initStyle(StageStyle.TRANSPARENT);
			stg.initModality(Modality.APPLICATION_MODAL);
			stg.show();
		} catch (IOException e) {
			util.tratamentoExcecao(e.getMessage().toString(), "[ CidadeController.actionBtnConfig() ]");
		}
		...
		if ((entidadeBean.getFlagAtivo().equals(1) && nivAcessoPermissao.getFlagDisable().equals(1))
			...
			if (util.showAlert(
					DadosGlobais.resourceBundle.getString("alertConfirmOprExcluir") + " "
							...
							+ " "
							+ DadosGlobais.resourceBundle.getString("cidadeController.alertConfirmOprExcluirCidade"),
					"confirmation")) {
				...
				Task<String> tarefaCargaPg = new Task<String>() {
					...
					protected void failed() {
						...
						util.tratamentoExcecao(exceptionProperty().getValue().toString(),
								"[ EstadoController.actionBtnDelete() ]");
						pBar.setProgress(0);
					}
				...
				t.setDaemon(true);
				t.start();
				stg = ProgressBarForm.showProgressBar(CidadeController.class, tarefaCargaPg,
	...
	void actionBtnInsert(ActionEvent event) {
		...
		if (nivAcessoPermissao.getFlagInsert().equals(1)) {
			entidadeBean = new Cidade();
			btnInsert.setDisable(true);
			btnSave.setDisable(false);
			btnCancel.setDisable(false);
			txtCodigo.setDisable(true);
			txtCodigo.setText("+1");
			Util.limpar(txtDescricao, txtCodIbge, txtCodUf, txtUf);
	...
	void actionBtnSave(ActionEvent event) {
		...
		if (nivAcessoPermissao.getFlagUpdate().equals(1)) {
			...
			if (!Util.noEmpty( lblErrorMessage, anchorPaneDetalhes, txtDescricao, txtCodUf, txtUf)) {

				entidadeBean.setDescricao(Util.textfieldNotNull("str", txtDescricao));
				entidadeBean.setCodigoIbge(Util.textfieldNotNull("str", txtCodIbge));
				//entidadeBean.setUf(uf);
				...
				if (txtCodigo.isDisable() && txtCodigo.getText().equals("+1")) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ CidadeController.actionBtnSave() - INSERT ]");
							pBar.setProgress(0);
						}
					...
					t.setDaemon(true);
					t.start();
					stg = ProgressBarForm.showProgressBar(CidadeController.class, TarefaRefresh,
				...
				if (entidadeBean.getCodigo().toString().equals(txtCodigo.getText())) {
					...
					Task<String> TarefaRefresh = new Task<String>() {
						...
						protected void failed() {
							stg.close();
							util.tratamentoExcecao(exceptionProperty().getValue().toString(),
									"[ CidadeController.actionBtnSave() - UPDATE ]");
							pBar.setProgress(0);
						}
	...
	@FXML
	void actionTxtCodUf(ActionEvent event) {
	...
	@FXML
	void keyPressedTxtCodUf(KeyEvent event) {
...
//		flagOpenBuscador = true;
	...
	@FXML
	void mouseClickedLblCodUf(MouseEvent event) {

		try {

			if (Util.getNivelAcessoEntidade(EnumNivelAcesso.PAIS).getFlagView().equals(1)) {
				util.crearTab(tabPrincipal, DadosGlobais.resourceBundle.getString("miCadUF"),
						"/views/vendas/viewEstado.fxml",
						new PaisController(Util.getNivelAcessoEntidade(EnumNivelAcesso.UF), tabPrincipal), false);
			} else {
				util.showAlert(DadosGlobais.resourceBundle.getString("alertInfPermicoes"), "warning");
			}
		} catch (IOException e) {
	 ...
	 *            DOS REGISTROS
	 */
	public void searchUf(int valorBusca) {
		...
		Task<String> TarefaSerch = new Task<String>() {
			...
			protected String call() throws Exception {
				...
				logRet = ufDAO.getById(valorBusca);
				return "-";
			}
			...
			protected void succeeded() {
				...
				if (logRet.getStatus().equals(EnumLogRetornoStatus.ERRO)) {
					Util.setStyleError(true, txtCodUf);
					txtCodUf.clear();
					txtUf.clear();
				} else {
					...
					if (logRet.getStatus().equals(EnumLogRetornoStatus.SUCESSO)) {
						txtUf.setText(((Uf) logRet.getObjeto()).getDescricao());
						entidadeBean.setUf((Uf) logRet.getObjeto());
	...
	public void showSearchUf() {
		...
		util.showSearchView("Cidade", "descricao", UfDAO.class, list, txtCodUf, txtUf, null, txtCodigo);
	...
	public void taskQuery(String tipoConsulta, boolean flagRefresh) {
		...
		Task<String> TarefaRefresh = new Task<String>() {
			...
			protected void failed() {
				stg.close();
				util.tratamentoExcecao(exceptionProperty().getValue().toString(),
						"[ CidadeController.TarefaRefresh() ]");
				pBar.setProgress(0);
			}
	...
	public void printExportShow() throws NoSuchFieldException, SecurityException {
		...
		try {
			...
			Scene scene = new Scene(
					util.openWindow("/views/utils/viewPrintExport.fxml",
							new PrintExportController(tbView, tableShowPrintList,
									DadosGlobais.resourceBundle.getString("cidadeController.lblTitleCidade"), pBar,
									stg)));
			...
			util.tratamentoExcecao(e.getMessage().toString(), "[ CidadeController.printExportShow() ]");
		}
	...
	public void updateTbView(String flagOperacao) {
		...
		t.setDaemon(true);
		t.start();
		stg = ProgressBarForm.showProgressBar(CidadeController.class, TarefaAtualiza,
		...
		if ((!flagLastRegistro && codigo > 0) || (flagLastRegistro)) {
			...
			Task<String> TarefaRefresh = new Task<String>() {
				...
				protected void succeeded() {
					...
					entidadeBean = (Cidade) logRet.getObjeto();
					...
					if (entidadeBean != null) {
						...
						txtCodIbge.setText(entidadeBean.getCodigoIbge());
						txtCodUf.setText(entidadeBean.getUf().getCodigo().toString());
						txtUf.setText(entidadeBean.getUf().getDescricao());
						...
						btnInsert.setDisable(false);
						txtCodigo.requestFocus();
						Util.limpar(txtDescricao, txtCodigo, txtCodIbge, txtCodUf, txtUf);
					}
				...
				protected void failed() {
					stg.close();
					util.tratamentoExcecao(exceptionProperty().getValue().toString(),
							"[ CidadeController.loadByID() - CidadeController.getLast() ]");
					pBar.setProgress(0);
				}
	...
	public void saveColumnSettings() {
		...
		xml.WriteXMLColumns(userDefinedColumnList, "Grid-Cad-Cidade");
	...
	public void configTableView() {
		...
		ArrayList<UserDefinedColumn> userDefinedColumns = xmlTableColumns.ReadXMLColumns("Grid-Cad-Cidade");
			...
			for (int i = 0; i < userDefinedColumns.size(); i++) {
				for (int j = 0; j < tbView.getColumns().size(); j++) {
					if (tbView.getColumns().get(j).getId().equals(userDefinedColumns.get(i).getName())) {
						TableColumn<Cidade, ?> tableColumn = tbView.getColumns().get(j);
	...
	public CidadeController(NivelAcesso nivAcesso, TabPane o_tabPrincipal) {
		// TODO Auto-generated constructor stub
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		Util.customSearchTextField("right", 90, txtCodUf);
		//Util.customSearchTextField("right", null, txtUf);
		...
		Util.setKeyPressDefaultStyles(txtCodigo, txtDescricao, txtCodUf, txtUf);
		
		Util.setStyleOnFocus(txtFilterColumn,txtCodigo, txtDescricao, txtCodIbge, txtUf,txtCodUf);

		Util.onlyNumbers(txtCodigo, txtCodUf, txtCodIbge);

		Util.onlyAlphanumeric(txtFilterColumn, txtDescricao, txtUf);
		...
		Util.whriteUppercase(txtFilterColumn, txtDescricao, txtUf);
		...
		tbColCodigo.setCellValueFactory(new PropertyValueFactory<Cidade, Integer>("codigo"));
		tbColDescricao.setCellValueFactory(new PropertyValueFactory<Cidade, String>("descricao"));
		tbColCodIbge.setCellValueFactory(new PropertyValueFactory<Cidade, String>("codigoIbge"));
		tbColUf.setCellValueFactory(new PropertyValueFactory<Cidade, String>("ufSigla"));
		tbColAtivoInat.setCellValueFactory(new PropertyValueFactory<Cidade, Integer>("flagAtivo"));
		...
		tbView.setRowFactory(tb -> {
			...
			// TENHA O FLAGATIVO = 0
			TableRow<Cidade> row = new TableRow<Cidade>() {
				@Override
				public void updateItem(Cidade objeto, boolean empty) {
						...
						public void handle(ActionEvent e) {
							try {
								...
								util.tratamentoExcecao(e1.getMessage().toString(),
										"[ CidadeController.acaoRastrearItens() ]");
							}
						}
					});
		...
		txtCodUf.focusedProperty().addListener(new ChangeListener<Boolean>() {
			@Override
			public void changed(ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue,
					Boolean newPropertyValue) {
				if (!newPropertyValue) {
					if (!btnInsert.isPressed() && !btnClose.isPressed() && !btnCancel.isPressed() && flagSearch && !txtCodUf.getRight().isPressed()){
						if (!txtCodUf.getText().isEmpty()){
							searchUf(Integer.parseInt(txtCodUf.getText()));		
						}
					}
				}
			}
...
//		txtUf.focusedProperty().addListener(new ChangeListener<Boolean>() {
//			@Override
//			public void changed(ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue,Boolean newPropertyValue) {
//				if (!newPropertyValue) {
//						txtCodUf.setDisable(false);
//						txtUf.setEditable(false);
//						Util.customEditedTextField("right", null, txtUf);	
//				}
//			}
//		});
		...
		// TECLAS DE ATALHOS PARA O FORMULARIO
				...
				anchorPanePrincipal.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
					...
					case F2:
						if (anchorPaneDetalhes.isVisible() && !txtCodUf.isFocused()) {
							txtCodigo.setDisable(true);
							actionBtnClose(null);
						}
						flagPaneFilter = false;
